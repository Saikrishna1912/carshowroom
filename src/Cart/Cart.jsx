...
export const Cart = () => {
    ...
    let addBtn = event => {
        ...
        event.target.parentNode.querySelector('#cartQuantity').innerText = CarQuantity + 1;
        let p = event.target.parentNode.querySelector('#cartPrice').innerText;
        event.target.parentNode.querySelector('#totalPrice').innerText = `Total price = ${parseInt(p.replace(/,/g, "")) * (CarQuantity + 1)} L.E`;
        ...
        let updatedCart = savedCars.cars.map(obj => {
            if (obj.carId === carID) {
                return { ...obj, quantity: CarQuantity + 1 }
            } else {
                return obj;
            }
        });
                            ...
                            data.carsDetails.map((item, index) => (
                                <div className="cart mx-3" id={item.carID} key={index}>
                                    ...
                                    <div className="cart-img position-relative">
                                        <img src={`${item.carImg}`} width='200' height='200' alt={`Car ${item.carBrand} ${item.carName}`} />
